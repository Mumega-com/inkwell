...
/**
 * SOS Mirror memory adapter — vector memory via Mirror API.
...
export class SOSMemoryAdapter implements MemoryPort {
  ...
  async remember(content: string, metadata?: Record<string, unknown>): Promise<string> {
    const res = await fetch(`${this.mirrorUrl}/remember`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) throw new Error(`Mirror remember failed: ${res.status}`)
    const data = await res.json() as { id: string }
    return data.id
  }

  async recall(query: string, limit = 10): Promise<MemoryResult[]> {
    const res = await fetch(`${this.mirrorUrl}/recall`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${this.token}`,
        'Content-Type': 'application/json',
