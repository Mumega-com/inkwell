...
## v8.6 — OSS Release Readiness

- [x] S069 A1/A2 first pass: scrub hardcoded Inkwell Network URLs, analytics IDs, and production Cloudflare IDs from public paths.
- [x] Move live instance content, processed inbox artifacts, and instance config behind ignored `instances/private/`.
- [x] Make `npm run build` release-clean with empty public content collections ([#89](https://github.com/Mumega-com/inkwell/issues/89)).
- [x] Make Worker tests run in local mode without requiring Wrangler remote login: 47 passed, 1 skipped for static Worker env mutation limits ([#88](https://github.com/Mumega-com/inkwell/issues/88)).
- [x] Verify `create-inkwell` scaffold locally: create, install, build, and dev server start all pass ([#91](https://github.com/Mumega-com/inkwell/issues/91)).
- [x] Replace deployment-specific public pages and static files with generic framework-facing copy ([#87](https://github.com/Mumega-com/inkwell/issues/87)).
- [ ] Decide clean-history strategy before public announcement: `git filter-repo` or fresh initial public commit ([#90](https://github.com/Mumega-com/inkwell/issues/90)).
- [ ] Run Athena G69-A1, then stop for approval before creating the public repo.
- [ ] Run G69-A2 after public repo setup and before announcement.

See [docs/OSS-RELEASE-PLAN.md](docs/OSS-RELEASE-PLAN.md).
