...
function formatCell(value: unknown, format: Column['format']): string {
  ...
  switch (format) {
    case 'number':
      return getFormatter('en-CA', { notation: 'compact', maximumFractionDigits: 1 }).format(value as number)
    case 'currency':
      return getFormatter('en-CA', { style: 'currency', currency: 'CAD', maximumFractionDigits: 0 }).format(value as number)
    case 'percent':
