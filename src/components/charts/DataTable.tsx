...
function formatCell(value: unknown, format: Column['format']): string {
  ...
  switch (format) {
    case 'number':
      return getCompactFormatter({ maximumFractionDigits: 1 }).format(value as number)
    case 'currency':
      return getCurrencyFormatter({ currency: 'CAD', maximumFractionDigits: 0 }).format(value as number)
    case 'percent':
