...
import { getCurrencyFormatter, getCompactNumberFormatter } from '../../lib/formatters'
...
function formatCell(value: unknown, format: Column['format']): string {
  ...
  switch (format) {
    case 'number':
      return getCompactNumberFormatter({ maximumFractionDigits: 1 }).format(value as number)
    case 'currency':
