...
export function KnowledgeGraph({ nodes: initialNodes, edges }: KnowledgeGraphProps) {
  ...
  const simulate = useCallback(() => {
    ...
    // Bolt Optimization: Eliminate per-frame garbage collection from intermediate array allocation
    const nodeMap = new Map()
    for (let i = 0; i < nodes.length; i++) {
      nodeMap.set(nodes[i].slug, nodes[i])
    }

    // Bolt Optimization: Reduce O(N^2) repulsion to O(N^2 / 2) via Newton's Third Law
    for (let i = 0; i < nodes.length; i++) {
      const node = nodes[i]
