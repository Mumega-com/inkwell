...
export function KnowledgeGraph({
  ...
  useEffect(() => {
    nodesRef.current = initialNodes.map((n, i) => ({
      ...
      x:
        dimensions.width / 2 +
        Math.cos((i / initialNodes.length) * Math.PI * 2) * 200 +
        (Math.random() - 0.5) * 50,
      y:
        dimensions.height / 2 +
        Math.sin((i / initialNodes.length) * Math.PI * 2) * 150 +
        (Math.random() - 0.5) * 50,
      vx: 0,
      vy: 0,
    }));
    nodeMapRef.current = new Map(nodesRef.current.map((n) => [n.slug, n]));
  ...
  const simulate = useCallback(() => {
    ...
    // Optimization: Calculate node repulsion in O(N^2 / 2) time using Newton's third law.
