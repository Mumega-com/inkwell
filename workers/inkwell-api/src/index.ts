...
import { calendarRoutes } from '../../../plugins/content/routes-calendar'
...
function isAllowedOrigin(origin: string): boolean {
  ...
  for (const allowed of allowedOrigins) {
    ...
    if (allowed.includes('*')) {
      const escaped = allowed.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace('\\*', '[a-z0-9-]+')
...
// Install kernel adapters so plugins can use context ports instead of bindings.
app.use('*', adapterMiddleware)

// Public SEO utility route; must run before the static catch-all.
...
// Admin: manually trigger the scheduled flywheel (NETWORK_TOKEN required)
...
app.get('*', async (c) => {
  ...
  // Portal SPA fallback: the portal tenant always serves the SPA index
  ...
  (function(){
    if (!localStorage.getItem('inkwell_auth_token') && ${JSON.stringify(busToken)}) {
      localStorage.setItem('inkwell_auth_token', ${JSON.stringify(busToken)});
      localStorage.setItem('inkwell_tenant_slug', ${JSON.stringify(cfTenant ?? '')});
      localStorage.setItem('inkwell_api_url', ${JSON.stringify(saasUrl)});
    }
  })();
<\/script>`
      ...
      if (
        contentType === 'text/html; charset=utf-8' &&
        tenantSlug &&
        new URL(c.req.url).hostname.endsWith(`.${config.domain}`)
      ) {
...
<p><a href="https://${config.domain}">Powered by ${config.name}</a></p>
</div></body></html>`,
    200,
  )
})
