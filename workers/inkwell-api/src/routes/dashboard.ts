...
dashboardRoutes.post('/squads/:id/kpis/snapshot', async (c) => {
  // Verify caller is internal (MUMEGA_TOKEN)
...
// ── GET /sessions — active session listing for access review (MUMEGA_TOKEN) ──
...
// ── GET /compliance-summary (MUMEGA_TOKEN) ────────────────────────────────

dashboardRoutes.get('/compliance-summary', async (c) => {
  const token = c.req.header('Authorization')?.replace('Bearer ', '') ?? c.req.header('MUMEGA_TOKEN')
  if (!token || token !== c.env.MUMEGA_TOKEN) {
...
// ── GET /risk-acceptances (MUMEGA_TOKEN) ──────────────────────────────────
...
// ── POST /alerts/:alertId/accept-risk (MUMEGA_TOKEN) ─────────────────────
...
// ── PUT /portal-config — upsert portal config for a tenant (MUMEGA_TOKEN) ──

dashboardRoutes.put('/portal-config', async (c) => {
  const token = c.req.header('Authorization')?.replace('Bearer ', '')
    ?? c.req.header('MUMEGA_TOKEN')
  if (!token || token !== c.env.MUMEGA_TOKEN) {
    return c.json({ error: 'unauthorized' }, 401)
  }
...
// ── POST /sessions/:sessionId/revoke (MUMEGA_TOKEN) ───────────────────────
...
// ── POST /automation-config — configure per-tenant automation provider (MUMEGA_TOKEN) ──
//
...
// ── GET /audit — paginated audit log query (MUMEGA_TOKEN) ─────────────────
