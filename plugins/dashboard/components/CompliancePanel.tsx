...
export default function CompliancePanel() {
  ...
  useEffect(() => {
    ...
    const slug = localStorage.getItem('inkwell_tenant_slug') ?? ''
    setApiUrl(url)
    setAuthToken(token)
    setTenantSlug(slug)
              ...
              <tbody>
                {sessions.map(s => (
                  <tr key={s.id} className="border-b border-ink-border/50">
                    ...
                    <td className="py-2 pr-4 text-ink-muted">{getDateTimeFormatter(undefined).format(new Date(s.created_at))}</td>
                    <td className="py-2 pr-4 text-ink-muted">{getDateTimeFormatter(undefined).format(new Date(s.expires_at))}</td>
                    <td className="py-2">
                      <button
            ...
            {riskLog.map(r => (
              <div key={r.id} className="bg-ink-surface border border-ink-border rounded-lg p-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-ink-muted">{getDateTimeFormatter(undefined).format(new Date(r.accepted_at))}</span>
                </div>
