...
function apiFetch(url: string, options?: RequestInit): Promise<Response> {
  const token =
    typeof window !== 'undefined' ? (localStorage.getItem('inkwell_auth_token') ?? '') : ''
  return fetch(url, {
    ...options,
    headers: {
...
function formatCurrency(cents: number): string {
  return getCurrencyFormatter({ locales: 'en-US', currency: 'USD', minimumFractionDigits: 2 }).format(cents / 100)
}
...
export function LeaguePanel() {
  ...
  const apiUrl =
    typeof window !== 'undefined' ? (localStorage.getItem('inkwell_api_url') ?? '') : ''
  const currentSquadId =
    typeof window !== 'undefined' ? (localStorage.getItem('inkwell_squad_id') ?? '') : ''
