...
function formatCents(cents: number): string {
  return getNumberFormatter('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(cents / 100)
}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <MiniKPI title="Visitors Today" value={getNumberFormatter(undefined).format(analytics.visitors)} change={`${analytics.pageViews} page views`} icon="◆" />
                  ...
                  <TableBody>
                    {contracts.map(c => (
                      <TableRow key={c.reference}>
                        ...
                        <TableCell className="font-mono text-sm">{c.rate != null ? `$${getNumberFormatter(undefined).format(c.rate)}` : '—'}</TableCell>
